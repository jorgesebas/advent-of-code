    ...
    while cola:
        ...
        for dx, dy in movimientos:
            ...
            if 0 <= nx < max_x and 0 <= ny < max_y and not visitados[ny][nx] and laberinto[ny][nx] == '.':
                visitados[ny][nx] = True  # Marcar como visitado
...
def main():
    ...
    if (sys.argv[1]=='data.txt'):
        grid = generar_mapa(registro[:12], max_x, max_y)  # Solo usamos los primeros 12 bytes en el ejemplo
    elif(sys.argv[1]=='input.txt'):
        grid = generar_mapa(registro[:1024],max_x,max_y) #para primeros kilobyte (1024 bytes)
    imprimir_matrix(grid)
    resultado, visitado = encontrar_camino_mas_corto(grid, inicio, fin)
    if resultado == -1:
        print("No es posible llegar al destino.")
    else:
        print(f"El camino más corto tiene {resultado} pasos.")
