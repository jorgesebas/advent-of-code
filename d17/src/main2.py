...
# Función para verificar si la salida del programa es igual al programa
def es_programa_autocopiado(A, B, C, program):
    output = run_program(A, B, C, program)
    # Comprobar si la salida coincide con el programa
    return output == ','.join(map(str, program))

# Modificar la función `run_program` para ser reutilizable
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Desafio de Advent of Code dia 17 parte 2",
    ...
    # Probar valores para A hasta encontrar el que hace que el programa sea autocopiado
    A_inicial = 1
    while not es_programa_autocopiado(A_inicial, B, C, program):
        A_inicial += 1
        print(f'\rProbado valores de A: {A_inicial:<10} , por favor espere!', end='', flush=True)

    print(f"El valor mínimo para A es {A_inicial}")
